...
const Chat:React.FC<ChatProps> = ({ currentThread , myUID }) => {
    ...
    const [seenMessage, setSeenMessage] = useState<ThreadRef>();
            ...
            thread.forEach(threadItem => {
                if(source == "Server" &&  thread.length > 0 && dataFetchedOnSnapShotRef.current){
                    if(threadItem.changeType === "added"){
                        ...
                        // ! Expirement
                        // if(!threadItem.lastMessageUID.includes(user?.uid)){
                            // console.log(threadItem.lastMessageUID ,"!==", user?.uid,"==",threadItem.lastMessageUID !== user?.uid, "==", threadItem.lastMessageUID.includes(user?.uid))
    ...
    useEffect(()=>{
        if(seenMessage){
            let threadSeen: string[] = [];
            threadSeen = [...seenMessage.threadSeen];
            if(threadSeen.includes(user!.uid!)){
                const indexOfSeenThread = chatThreadsValue.threads.findIndex(thread => {
                    if(thread.id === seenMessage.id){
                        return true
                    }
                })
                let threadx = chatThreadsValue.threads as [];
                let newThreadd: ThreadRef[] = [];
                threadx.map((threadxx, index) => {
                    if(index === indexOfSeenThread){
                        newThreadd.push(seenMessage)
                    } else {
                        newThreadd.push(threadxx)
                    }
                })
                setChatThreadsValue((prev) => ({
                    ...prev,
    ...
    },[seenMessage])//eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    }, [threadMessage])//eslint-disable-next-line react-hooks/exhaustive-deps
